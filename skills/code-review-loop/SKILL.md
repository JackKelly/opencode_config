...
1. **Station 1 (Math & ML Rigor):** 
   ...
   - **Phase 2 (Fresh Audit):**
     ...
     - If `total_flaws > 0`: 
       ...
       - Call `architect` to update the plan to `docs/temp/implementation_plan_v{Loop}.1_after_scientist.md`.
...
2. **Station 2 (Simplicity & Elegance):**
   ...
   - **Phase 2 (Fresh Audit):**
     ...
     - If `total_flaws > 0`:
       - If in "Mode 1" then pause to get the user's approval of the review.
       - Call `architect` to update the plan to `docs/temp/implementation_plan_v{Loop}.2_after_simplicity.md`.
        - Call `custom_build` to fix the code. Wait for completion, then use the `skill` tool to load the `implement` skill and follow its instructions to commit to git with a verbose and formatted message.
...
3. **Station 3 (Robustness & Testing):**
   ...
   - **Phase 2 (Fresh Audit):**
     ...
     - If `total_flaws > 0`:
       - If in "Mode 1" then pause to get the user's approval of the review.
       - Call `architect` to update the plan to `docs/temp/implementation_plan_v{Loop}.3_after_tester.md`.
        - Call `custom_build` to fix the code. Wait for completion, then use the `skill` tool to load the `implement` skill and follow its instructions to commit to git with a verbose and formatted message.
...
4. **Station 4 (Polish & Style):**
   ...
   - **Phase 2 (Fresh Audit):**
     ...
     - If `total_flaws > 0`:
       - If in "Mode 1" then pause to get the user's approval of the review.
       - Call `architect` to update the plan to `docs/temp/implementation_plan_v{Loop}.4_after_reviewer.md`.
        - Call `custom_build` to fix the code. Wait for completion, then use the `skill` tool to load the `implement` skill and follow its instructions to commit to git with a verbose and formatted message.
...
**Builder Pushback:** At any point, if `custom_build` reports that a flaw is impossible to implement, call `architect` to update the plan and reject the flaw with a technical justification (e.g., `docs/temp/implementation_plan_v{Loop}.X_after_builder_pushback.md`), then resume the current Station.
